...
const markdownIt = new MarkdownIt({
    ...
    highlight: function (str : string, lang : string) {
        ...
        let html = ""
        let copyBtn = `<button class="copy-btn" type="button" data-clipboard-action="copy" data-clipboard-target="#copy${codeIndex}"><el-icon class="bx bxs-copy"/></button>`
        ...
        if (lang && highLightJS.getLanguage(lang)) {
            try {
                ...
                html += preCode
                if (lineLength) {
                    copyBtn += '<b class="name">' + lang + '</b>'
                }
                // 将代码包裹在 textarea 中，由于防止textarea渲染出现问题，这里将 "<" 用 "&lt;" 代替，不影响复制功能
                return `<pre><div class="md-editor-code-header">${copyBtn}</div><code>${html}</code>${lineNum}</pre><textarea style="position: absolute;top: -9999px;left: -9999px;z-index: -9999;" id="copy${codeIndex}">${str.replace(/<\/textarea>/g, '&lt;/textarea>')}</textarea>`
...
markdownIt
    ...
    .use(toc, {
        ...
        linkClass: 'link',
        includeLevel: [1, 2, 3],
        callback: function (html: string, ast: TocAst) {
            if(ast.c.length != 0) {
                tocAst.push(ast)
